...
namespace OracleTest
{
   ...
   class FTP_Class
   {
       ...
       /// <returns></returns>  
       public string[] GetDirDetails(string localDir,string type)
       {
           ...
           try
           {
               infos = (Directory.GetFiles(localDir)); //获取当前目录的文件  

               //infos.Add(Directory.GetDirectories(localDir) ); //获取当前目录的目录  

               for (int i = 0; i < infos.Length; i++)
               {
                   int index = infos[i].LastIndexOf(@"\");
                   string aLastName = infos[i].Substring(infos[i].LastIndexOf(".") + 1, (infos[i].Length - infos[i].LastIndexOf(".") - 1)); //扩展名

                   if (aLastName.ToUpper() == type.ToUpper())
                   {
                       infos[i] = infos[i].Substring(index + 1);
                   }
